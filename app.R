...
dt_output = function(id, dt_title, dt_caption) {
    fluidRow(column(12, h2(dt_title), 
                    p(), 
                    p(dt_caption),
                    p()),
             DTOutput(id))
                      ...
                      navlistPanel(
                        ...
                        tabPanel(i18n$t("Fieldwork"),
                                 ...
                                 sliderInput("integer", i18n$t("Estimated number of fieldwork days:"),
                                             ...
                                             width = "100%"),
                                 p(),
                                 numericInput("obs", i18n$t("Estimated budget per fieldwork day, INCLUDING staff, equipment, consumables, rent etc. and EXCLUDING the cost of buying sampled medicines:"), 
                                              value = 10, min = 1, max = 100000000,
                                              width = "100%"),
...
server <- function(input, output) {
  ...
  prep.hr.fun <- reactive({
    ...
    observeEvent(input$fw_sampleframe_cell_edit, {
      ...
      # Without this line the table does not change but with it it jumps to row 1 after an edit.
      fw.samplingframe$data[i, j] <<- (DT::coerceValue(vl, fw.samplingframe$data[i, j]))
    })
    ...
    output$fw_perdayperoutlet <- renderDT({fw.samplecollectionrate$data %>%
        datatable(editable = list(target = "cell", 
                  ...
                  colnames = c(i18n$t("Outlet"),i18n$t("Samples per day"), i18n$t("Target number of samples"), i18n$t("Estimated days")))
    })
    ...
    observeEvent(input$fw_perdayperoutlet_cell_edit, {
      ...
      # Without this line the table does not change but with it it jumps to row 1 after an edit.
      fw.samplecollectionrate$data[i, j] <<- (DT::coerceValue(vl, fw.samplecollectionrate$data[i, j]))
    ...
    observeEvent(input$fw_medcost_cell_edit, {
      ...
      # Without this line the table does not change but with it it jumps to row 1 after an edit.
      fw.samplecost$data[i, j] <<- (DT::coerceValue(vl, fw.samplecost$data[i, j]))
      fw.samplecost$data[,3] <<- round(fw.samplecost$data[,2]*fw.samplecost$data[,1], 1)
    })
